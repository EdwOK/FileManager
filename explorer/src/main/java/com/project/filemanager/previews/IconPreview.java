...
public class IconPreview {
    ...
    private static DrawableCache<String> mMimeTypeIconCache;
    private static BitmapCache<String> mBitmapCache;
    ...
    public IconPreview(Activity activity) {
        ...
        mMimeTypeIconCache = new DrawableCache<>();
    ...
    private static boolean isvalidMimeType(File file) {
        boolean isImage = IconTypes.isPicture(file);
        boolean isVideo = IconTypes.isVideo(file);
    ...
    private static void loadFromRes(final File file, final ImageView icon) {
        ...
        if (file != null && file.isDirectory()) {
            ...
            if (mimeIcon == null) {
                final int mimeIconId = IconTypes.getIconForExt(fileExt);
                if (mimeIconId != 0) {
    ...
    private static Bitmap getPreview(File file) {
        final boolean isImage = IconTypes.isPicture(file);
        final boolean isVideo = IconTypes.isVideo(file);
